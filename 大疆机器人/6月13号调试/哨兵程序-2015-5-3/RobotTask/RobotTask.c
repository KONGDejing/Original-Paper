...
{
   SpeedNowOne = GetEncoder.V3; SpeedTagOne = Speed;   //无量纲   //GetEncoder.V3	CNT3
...
void PidSpeedControlTwo(s32 Speed)
{
   ...
   PidSpeedTypeTwo.kp      = MotorControl.User_Kp;                       //写入比例系数为15
   PidSpeedTypeTwo.ki      = MotorControl.User_Ki;                        //写入积分系数为5
   PidSpeedTypeTwo.kd      = MotorControl.User_Kd;                        //写入微分系数为5
...
{
   SpeedNowThr = GetEncoder.V4; SpeedTagThr = Speed;   //无量纲
   PidSpeedTypeThr.errNow = SpeedTagThr - SpeedNowThr; //计算并写入速度误差   	
   PidSpeedTypeThr.kp      = RM_Kp_1;                 //写入比例系数为15
   PidSpeedTypeThr.ki      = RM_Ki_1;                  //写入积分系数为5
   PidSpeedTypeThr.kd      = RM_Kd_1;                  //写入微分系数为5
   ...
   MotorSpeedSetOne(SpeedControlThr); 
	              //放入PWM，用于收敛速度的控制中
}

void PidSpeedControlFou(s32 Speed)
{
   SpeedNowFou = GetEncoder.V2; SpeedTagFou = Speed;   //无量纲
